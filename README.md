...
Upload a scanned PDF (multi-page) or a PNG/JPEG/WebP scan. Click a text box, then double-click (or press Enter) to edit it in place. Export the whole document as a PDF, or the current page as PNG or JPEG.
...
4. **Analyse typography by synthesis** for each edited element: candidate fonts are rendered, the scan is predicted as `paper·(1−α) + ink·α`, and size, scale, position, spacing, weight, slant and blur are optimized against the real pixels.
...
See `docs/` for details and `docs/ROADMAP.md` for current status and known limitations.
